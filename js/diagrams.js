	...
	Chart.controllers.WithLine = Chart.controllers.line.extend({
		draw: function (ease) {
			...
			if (video_player.readyState >= 3) {
				percentage = video_player.currentTime / video_player.duration;
			}
			chartMax = this.chart.chartArea.right - this.chart.chartArea.left;
			// draw line
			ctx.save();
			ctx.beginPath();
					...
					annotations: [
						{
							...
							mode: 'horizontal',
							scaleID: 'y-axis-0',
							value: body_parts_threshold[part.replace(' ', '')],
