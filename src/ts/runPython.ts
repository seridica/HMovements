...
export function processVideo(isCPU: number, options?: any): Promise<any> {
...
export function deidentifyVideo(configStore: ConfigStore) {
	return new Promise((resolve, reject) => {
		exec(`deidentification.exe "${configStore.videoPath}" "${configStore.skeletonPath}" "${configStore.savePath}"`, (err, data) => {
			if (err) {
				reject();
			} else {
				resolve(data);
